...
void draw() {
  ...
  for(float i = 0.0; i < N; i++){
     for(float j = 0.0; j < N; j++){
        beginShape();
        texture(sheep_texture);
        vertex(i * quad_size, j * quad_size, 0.0, i/N, j/N);
        vertex((i + 1) * quad_size, j * quad_size, 0.0, (i + 1)/N, j/N);
        vertex((i + 1) * quad_size, (j + 1) * quad_size, 0.0, (i + 1)/N, (j + 1)/N);
        vertex(i * quad_size, (j + 1) * quad_size, 0.0, i/N, (j + 1)/N);
        endShape();
   }
  }
  popMatrix();
